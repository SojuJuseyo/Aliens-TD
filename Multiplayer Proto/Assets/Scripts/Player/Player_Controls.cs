...
public class Player_Controls : MonoBehaviour {
	...
	private void cameraControls(){
		if (Input.GetAxis ("moveRight") > 0 && transform.position.x > minX) {
			//gauche
			transform.Translate(Vector3.left * Time.deltaTime * speedMove, Space.World);
		}
		if (Input.GetAxis ("moveRight") < 0 && transform.position.x < maxX) {
			//droite
			transform.Translate(Vector3.right * Time.deltaTime * speedMove, Space.World);
		}
		if (Input.GetAxis ("moveDown") < 0 && transform.position.z > minZ) {
			//bas
			transform.Translate(Vector3.back * Time.deltaTime * speedMove, Space.World);
		}
		if (Input.GetAxis ("moveDown") > 0 && transform.position.z < maxZ) {
			//haut
